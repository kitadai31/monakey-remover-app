...
ChMateのデータを削除することなく、鍵削除用APKの上書きインストールだけで鍵を削除します。(最初に一回アンインストールが必要ですが。)  
...
これを利用することで、データを保持したまま鍵削除アプリで上書きして、mateのアプリデータ内に格納された鍵を削除できます。  
...
これを利用して、ChMateと「同じパッケージ名」で「同じバージョンコード」の鍵削除アプリを作成し、それと「同じ署名」でChMateを再署名することで、データを維持しつつ2つのアプリを自由に入れ替えることができます。

ChMateのMonaKeyはアプリデータ領域の中に保存されているため、rootなしではどうあがいても他のアプリから消すことができませんが、
...
- root環境の人は？
  - この方法は非root向けです。rootならもっと便利にmate内から鍵を消せるようにするやつがあります。
...
## お知らせ
・2022/08/30  
いちいち削除ボタンを押す必要がない「nowindow」版を用意しました。  
nowindow版では、アプリを起動するとウィンドウを表示することなくMonaKeyの削除だけを行い即座に終了します。
